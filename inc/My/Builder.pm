  ...
  if (! $self->check_build_done_marker) {
    ...
    if($bp->{buildtype} eq 'use_config_script') {
      ...
      $self->set_ld_config($build_out);
        ...
        my $m = '';
        if ( $self->notes('travis') && $self->notes('travis') > 0  ) {
            # always select yes/all-messages for travis build
            $m = 1;
        }
        else {
            $m = $self->prompt(
...
#  my $p      = rel2abs($prefix);
  ...
  foreach my $full (@shlibs) {
    ...
    elsif ($f =~ /^(lib)?(intl|z|tiff|jpeg|png|webp|ogg|vorbis|vorbisfile|freetype|FLAC|mikmod)[^a-zA-Z]/) {
  ...
  for(qw(pthread  z jpeg tiff png webp ogg vorbis vorbisfile freetype
